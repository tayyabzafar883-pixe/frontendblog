...
const BlogData = ({ detail, likke, dislikee }) => {
  ...
  const deleteBlog = async (id) => {
    try {
      let resp = await fetch(` https://blog-backend-6kdu.vercel.app/api/post/delete/${id}`, {
        method: "DELETE",
        credentials: "include",
      });
  ...
  const like = async (action, id) => {
    try {
      let resp = await fetch(` https://blog-backend-6kdu.vercel.app/api/post/like/${id}`, {
        method: 'PUT',
        credentials: 'include',
      });
      let jresp = await resp.json();
  ...
  const disLike = async (action, id) => {
    try {
      let resp = await fetch(` https://blog-backend-6kdu.vercel.app/api/post/disLike/${id}`, {
        method: 'PUT',
        credentials: 'include',
      });
      let jresp = await resp.json();
